...
// import { Layers, MessageSquare, Zap, CheckSquare, ListChecks, CalendarDays, FileText, BarChart3, BrainCircuit, Image as ImageIcon, Palette, Users, Target as TargetIcon, AlarmClock as ClockIcon, CheckCircle, Loader2, XCircle, AlertCircle } from 'lucide-react';
// import ApiService from '@/apiService';

// const FeatureCard = ({ title, description, icon: Icon, status, onClick, projectData, notificationCount }) => {
//   const statusStyles = {
//     locked: "border-gray-600/50 bg-gray-800/30 text-gray-500 cursor-not-allowed",
//     available: "border-bizzwiz-electric-cyan/30 bg-bizzwiz-electric-cyan/10 text-bizzwiz-electric-cyan hover:bg-bizzwiz-electric-cyan/20 hover:border-bizzwiz-electric-cyan/50 cursor-pointer",
//     active: "border-bizzwiz-magenta-flare/50 bg-bizzwiz-magenta-flare/20 text-bizzwiz-magenta-flare cursor-pointer"
//   };
...
//       className={`relative p-6 rounded-xl border ${statusStyles[status]} transition-all duration-300 ease-in-out transform hover:scale-105 group h-full flex flex-col`}
//       onClick={status !== 'locked' ? onClick : () => toast({title: "Fonctionnalité Verrouillée", description: "Cette section sera disponible dans une future mise à jour.", variant: "destructive"})}
//     >
//       {notificationCount > 0 && status === 'available' && (
//         <motion.div 
//           initial={{scale:0, opacity:0}} animate={{scale:1, opacity:1}} transition={{delay:0.5, type:'spring', stiffness:200}}
//           className="absolute top-2 right-2 h-5 w-5 bg-red-500 rounded-full flex items-center justify-center text-xs font-bold text-white shadow-lg">
//           {notificationCount}
//         </motion.div>
//       )}
//       <div className="flex items-center mb-3">
//         <Icon size={24} className={`mr-3 transition-colors duration-300 ${status === 'available' ? 'group-hover:text-bizzwiz-star-white' : ''}`} />
//         <h3 className={`text-lg font-satoshi font-semibold ${status === 'available' ? 'group-hover:text-bizzwiz-star-white text-bizzwiz-comet-tail' : 'text-bizzwiz-star-white'}`}>{title}</h3>
//       </div>
//       <p className={`text-sm flex-grow ${status === 'available' ? 'text-bizzwiz-comet-tail/70 group-hover:text-bizzwiz-comet-tail' : 'text-bizzwiz-comet-tail/80'}`}>{description}</p>
//       {status === 'locked' && <p className="text-xs text-red-400/80 mt-2 pt-2 border-t border-gray-600/30">Bientôt disponible</p>}
//     </motion.div>
//   );
// };

// const ProjectOverviewCard = ({ project, onOpenModal }) => {
//   if (!project) return null;
//   const progress = project.roadmap?.reduce((acc, phase) => acc + phase.progress, 0) / (project.roadmap?.length || 1) || 0;

//   return (
//     <motion.div 
//       initial={{ opacity: 0, y: 20 }}
//       animate={{ opacity: 1, y: 0 }}
//       className="col-span-1 md:col-span-2 lg:col-span-3 bg-bizzwiz-glass-bg/60 backdrop-blur-lg p-6 rounded-2xl border border-bizzwiz-magenta-flare/20 shadow-xl"
//     >
//       <div className="flex flex-col md:flex-row gap-6">
//         <div className="md:w-1/3">
//           <h3 className="text-xl font-orbitron text-bizzwiz-magenta-flare mb-3">Aperçu du Projet</h3>
//           <img
//             src={project.mockup_link || "https://images.unsplash.com/photo-1581291518857-4e27b48ff24e?q=80&w=800&auto=format&fit=crop"}
//             alt={`Maquette de ${project.project_description || 'Projet'}`}
//             className="w-full h-48 object-cover rounded-lg border-2 border-bizzwiz-magenta-flare/30 shadow-lg cursor-pointer hover:opacity-90 transition-opacity"
...
//             <div className="flex items-center"><TargetIcon size={16} className="mr-2 text-bizzwiz-electric-cyan"/> <strong className="text-bizzwiz-comet-tail/80 mr-1">Mission:</strong> {project.mission_part1 || 'Non définie'}</div>
//             <div className="flex items-center"><Layers size={16} className="mr-2 text-bizzwiz-electric-cyan"/> <strong className="text-bizzwiz-comet-tail/80 mr-1">Type:</strong> {project.solution_type || 'Non défini'}</div>
//             <div className="flex items-center"><Palette size={16} className="mr-2 text-bizzwiz-electric-cyan"/> <strong className="text-bizzwiz-comet-tail/80 mr-1">Style:</strong> {project.visual_style || 'Non défini'}</div>
//             <div className="flex items-center"><Users size={16} className="mr-2 text-bizzwiz-electric-cyan"/> <strong className="text-bizzwiz-comet-tail/80 mr-1">Audience:</strong> {project.audience || 'Non définie'}</div>
//           </div>
//           <div>
//             <p className="text-sm text-bizzwiz-comet-tail mb-1">Progression Globale:</p>
//             <div className="w-full bg-bizzwiz-deep-space/50 rounded-full h-3.5 border border-bizzwiz-electric-cyan/20">
//               <motion.div 
//                 className="bg-gradient-to-r from-bizzwiz-electric-cyan to-bizzwiz-magenta-flare h-full rounded-full"
//                 initial={{ width: 0 }}
//                 animate={{ width: `${progress}%` }}
//                 transition={{ duration: 1, ease: "easeOut" }}
//               />
//             </div>
//             <p className="text-right text-xs text-bizzwiz-electric-cyan mt-1">{progress.toFixed(0)}%</p>
//           </div>
//         </div>
//       </div>
//     </motion.div>
...
// const InteractiveRoadmap = ({ roadmap, onOpenModal }) => {
//   if (!roadmap || roadmap.length === 0) {
//     return (
//       <div className="text-center py-8 text-bizzwiz-comet-tail">
//         <ListChecks size={32} className="mx-auto mb-2 opacity-50"/>
//         La feuille de route est en cours de préparation.
//       </div>
//     );
//   }

//   const getPhaseStatusIcon = (status) => {
//     if (status === 'Terminé') return <CheckCircle size={18} className="text-green-400"/>;
//     if (status === 'En cours') return <Loader2 size={18} className="text-blue-400 animate-spin"/>;
//     if (status === 'Bloqué') return <XCircle size={18} className="text-red-400"/>;
//     return <ClockIcon size={18} className="text-gray-500"/>;
//   };
...
//           className="bg-bizzwiz-deep-space/50 p-4 rounded-lg border border-bizzwiz-electric-cyan/20 hover:border-bizzwiz-magenta-flare/30 transition-colors cursor-pointer"
//           onClick={() => onOpenModal('roadmapDetail', { title: `Détails: ${phase.name}`, content: phase.description || "Aucun détail pour cette phase.", tasks: phase.tasks })}
//         >
//           <div className="flex justify-between items-center mb-2">
//             <h4 className="font-satoshi font-semibold text-bizzwiz-star-white">{phase.name}</h4>
//             <div className="flex items-center text-xs text-bizzwiz-comet-tail">
//               {getPhaseStatusIcon(phase.status)}
//               <span className="ml-2">{phase.status}</span>
//             </div>
//           </div>
//           <div className="w-full bg-bizzwiz-deep-space/30 rounded-full h-2 border border-bizzwiz-electric-cyan/10">
//             <motion.div 
//               className="bg-bizzwiz-electric-cyan h-full rounded-full"
//               initial={{ width: 0 }}
//               animate={{ width: `${phase.progress}%` }}
//               transition={{ duration: 0.8, ease: "circOut" }}
//             />
//           </div>
//           <p className="text-right text-xs text-bizzwiz-electric-cyan/80 mt-1">{phase.progress}%</p>
//         </motion.div>
//       ))}
//     </div>
//   );
...
//     return (
//       <div className="text-center py-10 text-bizzwiz-comet-tail">
...
//           animate={{ opacity: 1, y: 0 }}
//           className={`p-4 rounded-lg border ${task.status === 'Terminé' ? 'border-green-500/30 bg-green-500/10' : 'border-bizzwiz-electric-cyan/20 bg-bizzwiz-deep-space/50'}`}
//         >
//           <div className="flex items-start justify-between">
//             <div>
//               <h4 className={`font-satoshi font-semibold ${task.status === 'Terminé' ? 'text-green-400 line-through' : 'text-bizzwiz-star-white'}`}>{task.title}</h4>
//               {task.description && <p className={`text-xs mt-1 ${task.status === 'Terminé' ? 'text-bizzwiz-comet-tail/60 line-through' : 'text-bizzwiz-comet-tail/80'}`}>{task.description}</p>}
//               <p className="text-xs mt-2 text-bizzwiz-comet-tail/50">Assignée par: {task.assignedBy} - Le {new Date(task.createdAt).toLocaleDateString()}</p>
//             </div>
//             <div className="flex items-center space-x-2 pt-1">
//               <Label htmlFor={`task-${task.id}`} className="text-xs text-bizzwiz-comet-tail/70 cursor-pointer hover:text-bizzwiz-star-white">
...
//                 className={task.status === 'Terminé' ? 'border-green-500 data-[state=checked]:bg-green-500 data-[state=checked]:border-green-600' : 'border-bizzwiz-electric-cyan'}
//               />
//             </div>
//           </div>
//         </motion.div>
...
//     project_description: "Tableau de Bord Général",
//     roadmap: [],
//     assignedTasks: [],
//     mission_part1: "Gérer vos projets et ressources",
//     solution_type: "Tableau de bord",
//     visual_style: "Moderne",
...
//         } else {
//           const currentProject = response.data.find(p => p.id === parseInt(projectId));
//           if (currentProject) {
...
//           } else {
//             toast({ title: "Erreur", description: "Projet non trouvé ou accès non autorisé. Redirection...", variant: "destructive" });
//             navigate('/app/dashboard');
//           }
//         }
//       } else {
//         toast({ title: "Erreur", description: response.message || 'Failed to load projects', variant: "destructive" });
//         setProjectData(defaultProject);
//       }
//     } catch (error) {
//       console.error('Error fetching projects:', error);
//       toast({ title: "Erreur", description: error.message || 'An error occurred while fetching projects', variant: "destructive" });
//       setProjectData(defaultProject);
//     } finally {
//       setLoading(false);
//     }
...
//       setNewTaskNotificationCount(0); // Update notification count
//     }
//   };
...
//     if (projectId && projectId !== 'default-user-project') {
//       // Note: Task updates should be synced with the backend in a real scenario
//       toast({ title: "Statut de Tâche Mis à Jour", description: "Le statut de la tâche a été modifié.", className: "futuristic-toast-success" });
//     }
//   };

//   if (loading) {
//     return (
//       <div className="flex-1 flex items-center justify-center p-8 bg-bizzwiz-deep-space">
//         <div className="text-center">
//           <Zap size={48} className="mx-auto text-bizzwiz-electric-cyan animate-pulse mb-4" />
//           <p className="text-xl text-bizzwiz-star-white font-orbitron">Chargement de votre tableau de bord...</p>
//         </div>
//       </div>
//     );
//   }
...
//   return (
//     <div className="flex-1 p-4 sm:p-6 md:p-8 overflow-y-auto bg-bizzwiz-deep-space">
//       <motion.div
//         initial={{ opacity: 0, y: -20 }}
//         animate={{ opacity: 1, y: 0 }}
//         className="mb-8 p-6 bg-bizzwiz-glass-bg/50 backdrop-blur-lg rounded-xl border border-bizzwiz-electric-cyan/20 shadow-xl"
//       >
//         <h1 className="text-3xl sm:text-4xl font-orbitron text-bizzwiz-star-white mb-1">
//           Tableau de Bord : <span className="text-glow-electric-cyan">{projectData.project_description || 'Général'}</span>
//         </h1>
//         <p className="text-bizzwiz-comet-tail">Suivez l'évolution de votre projet et accédez à toutes vos ressources.</p>
//       </motion.div>

//       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
//         <ProjectOverviewCard project={projectData} onOpenModal={handleOpenModal} />
...
//           icon={ListChecks}
//           status={projectData.roadmap?.length > 0 ? "available" : "locked"}
//           onClick={() => handleOpenModal('roadmap', { title: "Feuille de Route Interactive", roadmap: projectData.roadmap || [] })}
...
//           description="Consultez les tâches assignées par votre administrateur et gérez leur avancement." 
//           icon={CheckSquare}
//           status={assignedTasks.length > 0 ? "available" : "locked"}
//           onClick={() => handleOpenModal('taskList', { title: "Gestion des Tâches Assignées", tasks: assignedTasks })}
//           projectData={projectData}
//           notificationCount={newTaskNotificationCount}
//         />
//         <FeatureCard 
//           title="Calendrier et Échéances" 
//           description="Visualisez les dates clés, les jalons et les livraisons de votre projet. (Bientôt disponible)" 
//           icon={CalendarDays}
//           status="locked"
//           onClick={() => handleFeatureClick("Calendrier et Échéances", "Ne manquez plus jamais une date importante. Visualisez votre planning projet sur un calendrier interactif.")}
//           projectData={projectData}
//           notificationCount={0}
//         />
//         <FeatureCard 
//           title="Documentation du Projet" 
//           description="Accédez à tous les documents, fichiers et ressources relatifs à votre projet. (Bientôt disponible)" 
//           icon={FileText}
//           status="locked"
//           onClick={() => handleFeatureClick("Documentation du Projet", "Centralisez tous vos documents importants : cahier des charges, spécifications, comptes rendus, etc.")}
//           projectData={projectData}
//           notificationCount={0}
//         />
//         <FeatureCard 
//           title="Statistiques et Suivi" 
//           description="Des graphiques et indicateurs pour suivre la performance et l'avancement. (Bientôt disponible)" 
//           icon={BarChart3}
//           status="locked"
//           onClick={() => handleFeatureClick("Statistiques et Suivi", "Obtenez des informations précieuses sur l'avancement de votre projet grâce à des indicateurs de performance clés et des graphiques.")}
//           projectData={projectData}
//           notificationCount={0}
//         />
//       </div>
...
//             projectName={projectData.project_description || 'Général'}
//           />
//         )}
//       </AnimatePresence>
...
//             <DialogContent className="bg-bizzwiz-deep-space border-bizzwiz-magenta-flare text-bizzwiz-star-white max-w-lg sm:max-w-xl md:max-w-2xl">
//               <DialogHeader>
//                 <DialogTitle className="font-orbitron text-bizzwiz-electric-cyan text-2xl">{modalContent.title}</DialogTitle>
//                 {modalContent.type === 'taskList' && <DialogDescription className="text-bizzwiz-comet-tail/70">Voici les tâches qui vous ont été assignées. Vous pouvez les marquer comme terminées.</DialogDescription>}
//               </DialogHeader>
//               <div className="py-4 max-h-[70vh] overflow-y-auto no-scrollbar">
//                 {modalContent.type === 'mockup' && (
//                   <img src={modalContent.imageUrl} alt="Maquette détaillée" className="w-full h-auto object-contain rounded-lg border border-bizzwiz-electric-cyan/20"/>
//                 )}
//                 {modalContent.type === 'roadmap' && (
//                   <InteractiveRoadmap roadmap={modalContent.roadmap} onOpenModal={handleOpenModal} />
//                 )}
...
//                   <div>
//                     <p className="text-bizzwiz-comet-tail mb-4">{modalContent.content}</p>
//                     {modalContent.tasks && modalContent.tasks.length > 0 && (
//                       <>
//                         <h5 className="font-satoshi font-semibold text-bizzwiz-electric-cyan mb-2">Tâches associées :</h5>
//                         <ul className="list-disc list-inside space-y-1 text-sm text-bizzwiz-comet-tail/90">
//                           {modalContent.tasks.map((task, i) => <li key={i}>{task.name} - <span className={`font-medium ${task.status === 'Terminé' ? 'text-green-400' : 'text-bizzwiz-electric-cyan/80'}`}>{task.status}</span></li>)}
//                         </ul>
//                       </>
//                     )}
//                     {(!modalContent.tasks || modalContent.tasks.length === 0) && <p className="text-sm text-bizzwiz-comet-tail/70">Aucune tâche spécifique listée pour cette phase.</p>}
//                   </div>
//                 )}
//                 {modalContent.type === 'featureInfo' && (
//                   <p className="text-bizzwiz-comet-tail">{modalContent.content}</p>
//                 )}
//               </div>
//               <DialogFooter>
//                 <Button onClick={() => setIsModalOpen(false)} className="futuristic-button bg-bizzwiz-magenta-flare hover:bg-bizzwiz-magenta-flare/80">Fermer</Button>
//               </DialogFooter>
//             </DialogContent>
//           </Dialog>
//         )}
